...
app=Flask(__name__)
app.config.update(MAX_CONTENT_LENGTH=MAX_UPLOAD_MB*1024*1024,SESSION_COOKIE_HTTPONLY=True,SESSION_COOKIE_SAMESITE="Lax",SESSION_COOKIE_SECURE=os.getenv("SESSION_COOKIE_SECURE","false").lower()=="true")
...
def log_request_start():
 ...
 cfg=settings.load()
 if cfg.get("oidc_enabled") and not session.get("user") and not session.get("local_fallback"):return redirect(url_for("user_login",next=request_target()))
...
def tls_verify_value(enabled=None):
 if enabled is None:enabled=settings.load().get("verify_tls",True)
 elif not isinstance(enabled,bool):enabled=str(enabled).lower()=="true"
 if not enabled:return False
 bundle=(os.getenv("REQUESTS_CA_BUNDLE") or os.getenv("SSL_CERT_FILE") or "").strip()
 return bundle or True

def oidc_discovery(issuer,verify=None):
 issuer=(issuer or "").strip().rstrip("/")
 if not issuer:raise ValueError("Issuer URL requis")
 if verify is None:verify=tls_verify_value()
 response=requests.get(issuer+"/.well-known/openid-configuration",timeout=15,verify=verify);response.raise_for_status();meta=response.json()
...
def safe_next(value):
 ...
 if not value.startswith("/") or value.startswith("//"):return url_for("home")
 prefix=(request.script_root or "").rstrip("/")
 if prefix and value!=prefix and not value.startswith(prefix+"/"):value=prefix+value
 return value
...
def oidc_callback():
 ...
 try:
  ...
  session["user"]={"sub":str(claims.get("sub")),"name":str(claims.get("name") or claims.get("preferred_username") or claims.get("email") or claims.get("sub")),"email":str(claims.get("email") or "")};session.pop("local_fallback",None);target=safe_next(session.pop("login_next",url_for("home")));logger.info("OIDC user authenticated sub=%s name=%s",session["user"]["sub"],session["user"]["name"]);return redirect(target)
...
def local_access():
 ...
 session["local_fallback"]=True;session.pop("user",None);logger.warning("Local fallback session started remote_addr=%s",request.remote_addr or "unknown");return redirect(safe_next(session.pop("login_next",url_for("home"))))
...
@admin_required
def test_oidc():
 data=request.json or {};saved=settings.load();issuer=data.get("issuer") or saved.get("oidc_issuer");verify=tls_verify_value(data.get("verify_tls",saved.get("verify_tls",True)))
 try:
  meta=oidc_discovery(issuer,verify);return jsonify(success=True,issuer=meta.get("issuer"),authorization_endpoint=meta.get("authorization_endpoint"),token_endpoint=meta.get("token_endpoint"))
