...
def _split_text_element(tag,max_chars):
 text=tag.get_text("",strip=False);name=tag.name or "p";attrs=_tag_attrs(tag);overhead=len(_wrap(name,"",attrs))+64;budget=max(256,max_chars-overhead)
 if name=="pre":units=text.splitlines(keepends=True)
 else:
  units=re.split(r"(?<=[.!?])\s+|\n+",text);units=[x+(" " if not x.endswith(("\n"," ")) else "") for x in units if x]
 pieces=[];current=""
...
def _split_table(tag,max_chars):
 ...
 header=[];thead=tag.find("thead")
 if thead:header=[str(row) for row in thead.find_all("tr")]
 elif rows and rows[0].find("th"):header=[str(rows[0])]
 header_ids={id(row) for row in (thead.find_all("tr") if thead else ([rows[0]] if header else []))};data=[str(row) for row in rows if id(row) not in header_ids];attrs=_tag_attrs(tag);head="".join(header);overhead=len(_wrap("table",head,attrs))+64;budget=max(512,max_chars-overhead);parts=[];current=[];size=0
 for row in data:
  if len(row)>budget:
...
def _split_list(tag,max_chars):
 ...
 attrs=_tag_attrs(tag);overhead=len(_wrap(tag.name,"",attrs))+64;budget=max(512,max_chars-overhead);parts=[];current=[];size=0
...
def _request_improvement(masked_html,url,model,key,level,custom_enabled,endpoint,request_json,response_path,chunk_index=1,chunk_total=1):
 ...
 else:payload={"model":model,"temperature":0.1,"messages":[{"role":"system","content":SYSTEM_PROMPT},{"role":"user","content":user_content}]};target=url.rstrip("/")+"/chat/completions"
 detail("AI request mode=%s endpoint=%s response_path=%s chunk=%s/%s",("custom" if custom_enabled else "openai"),target,response_path,chunk_index,chunk_total);response=None
 for attempt in range(AI_MAX_RETRIES+1):
  ...
  delay=AI_RETRY_BACKOFF*(2**attempt);detail("AI transient gateway error status=%s chunk=%s/%s attempt=%s/%s retry_in_seconds=%s",response.status_code,chunk_index,chunk_total,attempt+1,AI_MAX_RETRIES+1,delay)
...
def improve(html,url,model,key="",level="structure",send_images=False,custom_enabled=False,endpoint="/chat/completions",request_json="",response_path="choices.0.message.content",chunk_tokens=None):
 ...
 requested=min(MAX_INPUT_TOKENS,max(500,requested));chunk_token_budget=max(500,requested-reserved_tokens);max_chars=chunk_token_budget*4;estimated_tokens=_estimate_tokens(SYSTEM_PROMPT)+_estimate_tokens(masked_html)+300;chunks=_split_html(masked_html,max_chars) if estimated_tokens>requested else [masked_html]
 detail("AI improve model=%s level=%s send_images=%s original_chars=%s payload_chars=%s images_masked=%s estimated_tokens=%s chunks=%s token_limit=%s chunk_target=%s",model,level,send_images,len(html),len(masked_html),len(images),estimated_tokens,len(chunks),MAX_INPUT_TOKENS,requested);results=[]
