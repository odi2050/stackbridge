...
HEADING_TAGS={"h1","h2","h3","h4","h5","h6"};ATOMIC_TAGS={"table","pre","ul","ol"}
def _mask_images(html):
 images=[]
 def replace(match):index=len(images);images.append(match.group(0));return f'<span data-bookstack-image="{index}">IMAGE_BOOKSTACK_{index}</span>'
...
def _split_text_element(tag,max_chars):
 text=tag.get_text("",strip=False);name=tag.name or "p";attrs=_tag_attrs(tag);budget=max(256,max_chars-len(_wrap(name,"",attrs))-64);units=text.splitlines(keepends=True) if name=="pre" else [x+(" " if not x.endswith(("\n"," ")) else "") for x in re.split(r"(?<=[.!?])\s+|\n+",text) if x];pieces=[];current=""
...
def _split_table(tag,max_chars):
 ...
 thead=tag.find("thead");header=[str(row) for row in thead.find_all("tr")] if thead else ([str(rows[0])] if rows and rows[0].find("th") else []);header_ids={id(row) for row in (thead.find_all("tr") if thead else ([rows[0]] if header else []))};data=[str(row) for row in rows if id(row) not in header_ids];attrs=_tag_attrs(tag);head="".join(header);budget=max(512,max_chars-len(_wrap("table",head,attrs))-64);parts=[];current=[];size=0
 for row in data:
  if len(row)>budget:
...
def _stream_openai(response):
 content_type=(response.headers.get("Content-Type") or "").lower()
 if "text/event-stream" not in content_type:
  detail("AI stream requested but server returned content_type=%s; reading JSON response",content_type)
  return _response_value(response.json(),"choices.0.message.content")
 parts=[];events=0
 for raw in response.iter_lines(decode_unicode=True):
  ...
  if text:parts.append(text);events+=1
 detail("AI stream complete events=%s output_chars=%s",events,sum(map(len,parts)))
...
def _request_improvement(masked_html,url,model,key,level,custom_enabled,endpoint,request_json,response_path,chunk_index=1,chunk_total=1):
 ...
 prompts={"light":"Nettoyage léger uniquement.","structure":"Restructure avec H2/H3, listes, tableaux et blocs de code.","enrich":"Restructure et ajoute seulement un court résumé déductible de la source."};chunk_note=(f"\nCe contenu est la partie {chunk_index}/{chunk_total} d'un document. Améliore uniquement cette partie. Le titre de section peut être répété uniquement pour fournir le contexte: ne le duplique pas inutilement dans le résultat. Ne fusionne pas, ne complète pas et n'invente pas les données d'un tableau, d'une liste ou d'un bloc de code provenant d'une autre partie." if chunk_total>1 else "");user_content=prompts.get(level,prompts["structure"])+chunk_note+"\n"+masked_html
 streaming=not custom_enabled
 if custom_enabled:
 ...
 detail("AI request mode=%s streaming=%s endpoint=%s response_path=%s chunk=%s/%s",("custom" if custom_enabled else "openai"),streaming,target,response_path,chunk_index,chunk_total);response=None
 for attempt in range(AI_MAX_RETRIES+1):
  response=request("POST",target,headers=headers,json=payload,timeout=AI_REQUEST_TIMEOUT,stream=streaming)
  if response.ok:break
  if response.status_code not in AI_RETRYABLE_STATUS or attempt>=AI_MAX_RETRIES:_raise_api_error(response)
  response.close();delay=AI_RETRY_BACKOFF*(2**attempt);detail("AI transient gateway error status=%s chunk=%s/%s attempt=%s/%s retry_in_seconds=%s",response.status_code,chunk_index,chunk_total,attempt+1,AI_MAX_RETRIES+1,delay)
  if delay:time.sleep(delay)
 _raise_api_error(response)
 try:return _stream_openai(response) if streaming else _response_value(response.json(),response_path)
...
def improve(html,url,model,key="",level="structure",send_images=False,custom_enabled=False,endpoint="/chat/completions",request_json="",response_path="choices.0.message.content",chunk_tokens=None):
 masked_html,images=(html,[]) if send_images else _mask_images(html);reserved_tokens=max(800,_estimate_tokens(SYSTEM_PROMPT)+300)
 try:requested=int(chunk_tokens or MAX_INPUT_TOKENS)
 except (TypeError,ValueError):requested=MAX_INPUT_TOKENS
 requested=min(MAX_INPUT_TOKENS,max(500,requested));chunk_token_budget=max(500,requested-reserved_tokens);max_chars=chunk_token_budget*4;estimated_tokens=_estimate_tokens(SYSTEM_PROMPT)+_estimate_tokens(masked_html)+300;chunks=_split_html(masked_html,max_chars) if estimated_tokens>requested else [masked_html];detail("AI improve model=%s level=%s send_images=%s original_chars=%s payload_chars=%s images_masked=%s estimated_tokens=%s chunks=%s token_limit=%s chunk_target=%s",model,level,send_images,len(html),len(masked_html),len(images),estimated_tokens,len(chunks),MAX_INPUT_TOKENS,requested);results=[]
 for index,chunk in enumerate(chunks,1):
  detail("AI chunk start chunk=%s/%s chars=%s estimated_tokens=%s",index,len(chunks),len(chunk),_estimate_tokens(chunk));result=_request_improvement(chunk,url,model,key,level,custom_enabled,endpoint,request_json,response_path,index,len(chunks));results.append(_validate_chunk_result(result));detail("AI chunk complete chunk=%s/%s output_chars=%s",index,len(chunks),len(result))
 content="\n".join(results)
 if images:content=_restore_images(content,images)
 content=_validate_chunk_result(content);detail("AI improve complete output_chars=%s images_restored=%s chunks=%s",len(content),len(images),len(chunks));return content
