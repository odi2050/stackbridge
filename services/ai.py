...
def improve(html,url,model,key="",level="structure",send_images=False,custom_enabled=False,endpoint="/chat/completions",request_json="",response_path="choices.0.message.content",chunk_tokens=None):
 ...
 for index,chunk in enumerate(chunks,1):
  chunk_started=time.monotonic();detail("AI chunk start chunk=%s/%s chars=%s estimated_tokens=%s",index,len(chunks),len(chunk),_estimate_tokens(chunk));result=_request_improvement(chunk,url,model,key,level,custom_enabled,endpoint,request_json,response_path,index,len(chunks));results.append(_validate_chunk_result(result));detail("AI chunk complete chunk=%s/%s output_chars=%s duration_ms=%s",index,len(chunks),len(result),round((time.monotonic()-chunk_started)*1000))
 content="\n".join(results)
 if images:content=_restore_images(content,images)
 content=_validate_chunk_result(content);detail("AI improve complete output_chars=%s images_restored=%s chunks=%s total_duration_ms=%s",len(content),len(images),len(chunks),round((time.monotonic()-total_started)*1000));return content
